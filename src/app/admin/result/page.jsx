...
export default function Result() {
  ...
  const getResult = async () => {
    try {
      // Build the query based on whether semester is provided
      const query = db.select({
        ...getTableColumns(Students),
      }).from(Students).where(eq(Students.batch, batch));

      if (sem) {
        query.where(eq(Students.sem, sem));
      }
  ...
  const handlePromote = async (student) => {
    try {
      ...
      if (parseInt(newSemester) <= 6) {
        ...
        alert("Semester must be 6 or less");
      }
    } catch (error) {
                ...
                {data.map((student, index) => (
                  <tr key={index} className='hover:bg-gray-100 dark:hover:bg-gray-700'>
                    <td className="px-2 md:px-4 py-2">
                      <Link href={`/admin/result/${student.uucms}`}>
                        {student.name}
                      </Link>
                    </td>
