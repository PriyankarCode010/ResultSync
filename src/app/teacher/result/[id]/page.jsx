...
const Page = ({ params }) => {
  ...
  useEffect(() => {
    const fetchStudent = async () => {
      try {
        const result = await db.select().from(Students).where(eq(Students.uucms, params.id)).execute();
  ...
  return (
    <div className='p-10'>
      <div className='flex items-center justify-between'>
        ...
        <h2 className=' font-extrabold text-xl'>{params?.id}</h2>
      ...
      <MainResultPage semester={selectedSem} id={params.id}/>
    </div>
  );
};
