...
import { Students, Sem2 } from '../utils/schema';
...
export default function Sem2Report({ batch }) {
  ...
  const subjectNames = ["ENG", "HIN/KA", "DST", "WPG", "NSM", "DSL", "NSL", "WPL", "HES", "ECA"];

  const fetchData = async () => {
    try {
      const result = await db.select({
        ...getTableColumns(Students),
        ...getTableColumns(Sem2)
      })
      .from(Sem2)
  ...
  return (
    <div className='p-9'>
      <h3 className='flex gap-5 text-3xl font-bold'>Batch :-  {batch}  {"        Semester :- 2"}</h3>
