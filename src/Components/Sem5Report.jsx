...
export default function Sem5Report({ batch }) {
  ...
  const subjectNames = ["SE", "SPT", "CYS", "Pyth", "C#", "A/Pyth Lab", "C# Lab"];

  const fetchData = async () => {
    try {
      const result = await db.select({
        ...getTableColumns(Students),
        ...getTableColumns(Sem5)
      })
      .from(Sem5)
      .leftJoin(Students, eq(Students.uucms, Sem5.uucms))
      .where(eq(Students.verified, true))
      .where(eq(Sem5.batch, batch))
      .execute();
  ...
  return (
    <div className='p-9'>
      <h3 className='flex gap-5 text-3xl font-bold'>Batch :-  {batch}  {"        Semester :- 5"}</h3>
