...
import { Sem3 } from '@/utils/schema';
import { Loader } from 'lucide-react';
import { eq } from 'drizzle-orm';
import NivoBarChart from "@/Components/NivoBarChart";

const Sem3Component = ({ uucmsId, role,session }) => {
  ...
  useEffect(() => {
    if (uucms) {
      const fetchData = async () => {
        try {
          const record = await db.select().from(Sem3).where(eq(Sem3.uucms, uucms)).execute();
          if (record.length > 0) {
            const data = record[0];
            setBatch(data.batch);
            setMarks({
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const existingRecord = await db.select().from(Sem3).where(eq(Sem3.uucms, uucms)).execute();
      console.log('Existing Record:', existingRecord);
      
      if (existingRecord.length > 0) {
        console.log('Updating record...');
        await db.update(Sem3)
          .set({
            batch:uucms.slice(5,7),
            sub1: marks.kanAddEng,
            sub2: marks.distH,
            sub3: marks.oopTh,
            sub4: marks.ospTh,
            sub5: marks.datTh,
            lab1: marks.pr1PrJava,
            lab2: marks.pr2PrLinux,
            total: totalMarks,
            editedBy:lastEdit,
            per,
            status
          })
          .where(eq(Sem3.uucms, uucms))
          .execute();
        console.log('Record updated');
      } else {
        console.log('No existing record found, inserting new record...');
        await db.insert(Sem3)
          .values({
            uucms,
            batch:uucms.slice(5,7),
            sub1: marks.kanAddEng,
  ...
  const totalMarks = Object.values(marks).reduce((acc, mark) => acc + mark, 0);
  const totalPossibleMarks = 700; // Assuming the total possible marks is 750
