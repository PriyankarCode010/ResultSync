// components/Sem4Report.js

"use client";

import { db } from '../utils/dbConfig';
import { Students, Sem4 } from '../utils/schema';
import { eq, getTableColumns } from 'drizzle-orm';
import React, { useState, useEffect } from 'react';
import NivoPieChart from '../Components/NivoBarChart';
...
const checkIfFailed = (student) => {
  const subjects = ['sub1', 'sub2', 'sub3', 'sub4', 'sub5', 'lab1', 'lab2'];
...
export default function Sem4Report({ batch }) {
  ...
  const subjectNames = ["KA/Add.Eng", "DAA", "CAN", "A. JAVA", "DBMS", "JAVA PR1PR", "DBMS PR2PR"];

  const fetchData = async () => {
    try {
      const result = await db.select({
        ...getTableColumns(Students),
        ...getTableColumns(Sem4)
      })
      .from(Sem4)
      .leftJoin(Students, eq(Students.uucms, Sem4.uucms))
      .where(eq(Students.verified, true))
      .where(eq(Sem4.batch, batch))
      .execute();
  ...
  return (
    <div className='p-9'>
      <h3 className='flex gap-5 text-3xl font-bold'>Batch :-  {batch}  {"        Semester :- 4"}</h3>
