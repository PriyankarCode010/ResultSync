...
import { Students, Sem3 } from '../utils/schema';
...
export default function Sem3Report({ batch }) {
  ...
  const subjectNames = ["Kan/Add Eng", "DISTH", "OOPTH", "OSPTH", "DATTH", "PR1PR JAVA", "PR2PR Linux"];

  const fetchData = async () => {
    try {
      const result = await db.select({
        ...getTableColumns(Students),
        ...getTableColumns(Sem3)
      })
      .from(Sem3)
      .leftJoin(Students, eq(Students.uucms, Sem3.uucms))
      .where(eq(Students.verified, true))
      .where(eq(Sem3.batch, batch))
      .execute();
  ...
  return (
    <div className='p-9'>
      <h3 className='flex gap-5 text-3xl font-bold'>Batch :-  {batch}  {"        Semester :- 3"}</h3>
