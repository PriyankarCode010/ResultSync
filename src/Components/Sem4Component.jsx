...
const Sem4Component = ({ uucmsId, role,session }) => {
  ...
  useEffect(() => {
    if (uucms) {
      const fetchData = async () => {
        try {
          const record = await db.select().from(Sem4).where(eq(Sem4.uucms, uucms)).execute();
          if (record.length > 0) {
            const data = record[0];
            setBatch(data.batch);
            setMarks({
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const existingRecord = await db.select().from(Sem4).where(eq(Sem4.uucms, uucms)).execute();
      console.log('Existing Record:', existingRecord);
      
      if (existingRecord.length > 0) {
        console.log('Updating record...');
        await db.update(Sem4)
          .set({
